...
class TestBuildStructuralFramingParameters:
    ...
    def test_n_inputs_produce_n_outputs(self, gen):
        ...
        result = gen._build_structural_framing_parameters(beams, _GRID)
        assert len(result) == 2
    ...
    def test_z_at_storey_height(self, gen):
        beams = [_make_beam(0, 100, 290, 900, 310)]
        result = gen._build_structural_framing_parameters(beams, _GRID)
        assert result[0]["start_point"]["z"] == pytest.approx(gen.default_wall_height)
        assert result[0]["end_point"]["z"]   == pytest.approx(gen.default_wall_height)
