...
flowchart TB
    ...
    subgraph PIPE["🏗️ Pipeline"]
        ...
        Beam Z = Level 0 + slab thickness (beam top flush with slab top)
...
| 6 | Geometry generation | `GeometryGenerator` | `_px_to_world` → `_snap_to_nearest_grid` → Revit recipe; beam insertion Z = `Level0 + slab_thickness` so the beam top is flush with the slab top (body hangs `depth_mm` below into the foundation zone); skips admittance-rejected elements; reads `admittance_metadata.material` to emit `RCBeam…` / `SteelBeam…` family names |
